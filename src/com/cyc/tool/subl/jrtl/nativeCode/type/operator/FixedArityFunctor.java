...
public abstract class FixedArityFunctor implements CommonSymbols {
  ...
  final public static HashMap signaturesUncoded = new HashMap();
  final public static HashMap signaturesCoded = new HashMap();
  final public static ArrayList makeInstanceTraces = new ArrayList();
   ...
   * @param i
   */
  private static void addCount(Map map, String signature, int i) {
    // TODO Auto-generated method stub
    if (map != null) {
  ...
  static public SubLObject dump_instances() {
    ...
    int totalC = 0;
    Iterator<Map.Entry> inIteratorC = signaturesCoded.entrySet().iterator();
    while (inIteratorC.hasNext()) {
      Entry e = inIteratorC.next();
    ...
    int totalU = 0;
    Iterator<Map.Entry> inIteratorU = signaturesUncoded.entrySet().iterator();
    while (inIteratorU.hasNext()) {
      Entry e = inIteratorU.next();
      Integer v = (Integer) e.getValue();
      if (v == null || v == 0) {
        continue;
      }
