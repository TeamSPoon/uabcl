...
public abstract class AbstractSubLSymbol extends AbstractSubLObject implements SubLSymbol {
  ...
  AbstractSubLSymbol(SubLString symbolName, SubLPackage thePackage) {
    ...
    if ((thePackage == SubLPackage.KEYWORD_PACKAGE) && (!isKeyword())) { //@todo get rid of this test
      Errors.error("Got invalid keyword: " + this);
    }
  }
  ...
  public final void setProperty(SubLObject indicator, SubLObject value) {
    if (indicator == CommonSymbols.BINDING_TYPE) { // @hack
      Errors.error("Cannot set symbol property: " + indicator + " on symbol: " + this);
    }
    synchronized (plist) {
  ...
  public final void removeProperty(SubLObject indicator) {
    if (indicator == CommonSymbols.BINDING_TYPE) { // @hack
      Errors.error("Cannot remove symbol property: " + indicator + " from symbol: " + this);
    }
    synchronized (plist) {
  ...
  public static final SubLObject gensym(SubLObject val) {
    ...
    if (val == SubLNil.NIL) {
      ...
      throw new SubLException("Invalid argument to gensym -- exptected " 
  ...
  protected SubLObject value = UNBOUND; // @todo consider whether this should be volatile
  ...
  private static final Map symbolCanonicalizerCache = new LinkedHashMap(MAX_CANONICALIZER_CACHE_SIZE+1, .75F, true) {
