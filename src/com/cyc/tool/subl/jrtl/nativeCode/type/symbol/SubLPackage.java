...
public final class SubLPackage extends AbstractSubLObject implements SubLObject {
  ...
  public static final SubLPackage setCurrentPackage(SubLString packageName) {
    ...
    if (thePackage == null) {
      throw new SubLException("Unabled to switch to package: " + packageName + ".");
    }
  ...
  private void init(SubLString name, SubLList useList, SubLList nickNames) {
    if ((name == null) || ("".equals(name))) {
      throw new SubLException("Got invalid package name.");
    }
    ...
    synchronized (PACKAGE_LOCK) {
      ...
      if (packageNameToPackageMap.get(name) != null) {
        throw new SubLException("A package named " + name + " already exists.");
      }
      this.name = name;
      ...
      for (SubLObject cur = nickNames; cur != SubLNil.NIL; cur = cur.rest()) {
        ...
        if (packageNameToPackageMap.get(nickName) != null) {
          throw new SubLException("A package named " + nickName + " already exists.");
        }
        if (!nickName.isString()) {
          throw new SubLException("Package nick names must be strings, got: " + nickName);
        }
      ...
      for (SubLObject cur = useList; cur != SubLNil.NIL; cur = cur.rest()) {
        ...
        try {
          if (curPackageName.isString()) {
            ...
            throw new SubLException("Got invalid package spec: " + curPackageName);
          }
        } catch (Exception e) {
          Errors.handleError("Error while creating a package.", e);
        }
      ...
      for (SubLObject cur = nickNames; cur != SubLNil.NIL; cur = cur.rest()) {
        try {
          ...
          Errors.handleError("Error while attempting to add package nickname: " + curNickName, e);
        }
      }
    }
    for (SubLObject cur = useList; cur != SubLNil.NIL; cur = cur.rest()) {
  ...
  private Map<SubLString, SubLSymbol> symbolNameToSymbolMap = new HashMap();
  private static SubLList allPackages;
  private static final HashMap packageNameToPackageMap = new HashMap(32);
  private static final HashMap packageNameToPackageMapNative = new HashMap(32);
