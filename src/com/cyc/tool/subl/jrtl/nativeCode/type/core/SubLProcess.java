...
public abstract class SubLProcess extends SafeRunnable implements Runnable, SubLObject, CommonSymbols {
  ...
  public final void addInterrupt(SubLObject function) {
    addInterrupt(new Thunk(function));
  }
  ...
  public final void killProcess() {
    addInterrupt(Thunk.requestTermination());
  }
  ...
  static public class TerminationRequest extends RuntimeException {
  ...
  static public class Thunk {
    public Thunk( SubLObject function, SubLObject[] arguments) {
      this.function = function;
      this.arguments = arguments;
      sender = Thread.currentThread();
    }
    
    public Thunk( SubLObject function) {
      this(function, Resourcer.EMPTY_SUBL_OBJECT_ARRAY);
    }
    
    static public Thunk requestTermination() {
      return new Thunk(KILL_KEYWORD);
    }
  ...
  private boolean isPossiblyStillborn = false;
  ...
  private SubLString whoState = Threads.UNSET_STRING;
  private volatile SubLProcessState state = DEAD_STATE;
  private List interrupts = Collections.synchronizedList(new ArrayList());
  
  private static final HashSet currentProcesses = new HashSet(128);
