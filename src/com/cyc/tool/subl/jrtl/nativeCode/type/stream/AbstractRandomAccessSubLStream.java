...
public abstract class AbstractRandomAccessSubLStream extends AbstractSubLStream {
    ...
    if (size < tmpBuffer.length) {
      try {
        ...
        for (int i = 0; i < size; i++) {
          curByte = tmpBuffer[i];
          if (curByte == -1) { // EOF
            throw new SubLException("Got unexpected end of file on stream: " + this);
          }
  ...
  public long readByteSequenceToPositiveInteger(final int bytesInInteger) {
    if ((bytesInInteger > 8) || (bytesInInteger < 0)) {
      throw new SubLException("Bytes in integer is bad: " + bytesInInteger);
    }
    long result = 0;
    byte[] tmpBuffer = SubLProcess.currentSubLThread().byteBuffer;
    try {
        ...
        synchronized (SubLThread.getInterruptLock()) {
          ...
          try {
            if (fileChannel != null) {
              ...
              try { fileChannel.force(true); } catch (Exception e) {} //ignore errors
              fileChannel.close();
              fileChannel = null;
            }
            if (raFile != null) {
  ...
  public final void seek(long pos) {
    ...
    try {
      if (writeByteBuffer != null) {
        ...
        long maxBytePos = getUnderlyingFilePos();
        long minBytePos = maxBytePos - byteBuffer.limit();
        if ((pos >= minBytePos) && (pos < maxBytePos)) {
          byteBuffer.position((int)(pos - minBytePos));
          setInputIndex(pos);
          return;
        }
  ...
  public void write(int c) {
    try {
      if ((c < 0) || (c > 255)) {
        throw new SubLException("Non-ascii characters not currently supported!");
      }
  ...
  public void writeChar(char c) {
    try {
      if ((c > 255) || (c < 0)) { // @hack 
        throw new SubLException("Non-ascii characters not currently supported: " + c + ".");
      }
      writeByteBuffer.put((byte)c);
    } catch (BufferOverflowException boe) {
      flush();
  ...
  private static final int BUF_SIZE = 8192 * 2;
