...
public class Eval implements SubLFile {
  ...
  public static final File openFileForReading(String filename) {
    ...
    if (!file.exists()) {
      throw new SubLException("File doesn't exist: " + filename);
    }
    if (file.isDirectory()) {
      throw new SubLException("Expected file but got directory: " + filename);
    }
    if (!file.canRead()) {
      throw new SubLException("File is unreadable: " + filename);
    }
    return file;
  }
  ...
  public static final SubLObject macroexpand(SubLObject form, SubLObject env) {
    if ((env == UNPROVIDED)) { env = NIL; }
    return Errors.unimplementedMethod("macroexpand");
  }
