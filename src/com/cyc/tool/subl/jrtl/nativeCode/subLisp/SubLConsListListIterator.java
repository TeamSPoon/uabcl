...
public final class SubLConsListListIterator implements SubLListListIterator, ListIterator {
  ...
  void setList(SubLList list) {
    if ((list != null) && (list.isArrayBased())) { 
      throw new SubLException("Can't create a cons iterator for an array list.");
    }
    this.list = list;
  }
  ...
  public Object next() {
    ...
    if (!cursorCdr.isList()) {
      throw new SubLException("Got unexpected dotted list item: " + cursorCdr);
    }
  ...
  public SubLObject getDottedElement() {
    if (cursorCdr.isList()) {
      throw new SubLException("Attempt to get dotted element from an iterator that isn't at the end of the list.");
    }
    return cursorCdr;
  }
  
  public void remove() {
    throw new SubLException("Unimplemented.");
    /*if (lastRet == -1) {
  ...
  public Object previous() {
    throw new SubLException("Can't iterate backwards on a cons list.");
  }
