 ...
 * $Id: StructureObject.java 11754 2009-04-12 10:53:39Z vvoutilainen $
 *
...
public class StructureObjectImpl extends AbstractLispObject implements StructureObject
{
  ...
  public StructureObjectImpl(Symbol symbol, LispObject[] slots)
    throws ConditionThrowable
  {
    structureClass = (StructureClass) findLispClass(symbol); // Might return null.
    this.slots = slots;
  }

  public StructureObjectImpl(Symbol symbol, LispObject obj0)
    throws ConditionThrowable
  {
    structureClass = (StructureClass) findLispClass(symbol); // Might return null.
  ...
  public StructureObjectImpl(Symbol symbol, LispObject obj0, LispObject obj1)
    throws ConditionThrowable
  {
    structureClass = (StructureClass) findLispClass(symbol); // Might return null.
    ...
    if (obj instanceof StructureObject)
      {
        ...
        for (int i = 0; i < slots.length; i++)
          {
            if (!slots[i].equalp(o.getSlotValue(i)))
              return false;
          }
        return true;
      }
  ...
  private static final Primitive COPY_STRUCTURE =
    new Primitive(SymbolConstants.COPY_STRUCTURE, "structure")
    {
      ...
      {
          if (arg instanceof StructureObject)
            return new StructureObjectImpl((StructureObject)arg);
          return type_error(arg, SymbolConstants.STRUCTURE_OBJECT);
      }
    };
}
