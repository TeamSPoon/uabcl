...
public final class Primitives extends LispFile
{
    ...
    new Primitive(SymbolConstants.FBOUNDP, "name")
    {
      ...
      public LispObject execute(LispObject arg) throws ConditionThrowable
      {
        ...
        if (isValidSetfFunctionName(arg))
          {
            LispObject f = get(arg.cadr(), SymbolConstants.SETF_FUNCTION, null);
            return f != null ? T : NIL;
          }
  ...
  public static final Primitive APPEND =
    new Primitive(SymbolConstants.APPEND, "&rest lists")
    {
        ...
        throws ConditionThrowable
      {
        ...
        Cons result = new Cons(first.first());
        Cons splice = result;
        first = first.rest();
        while (first != NIL)
          {
            Cons temp = new Cons(first.first());
            splice.cdr = temp;
            splice = temp;
            first = first.rest();
          }
        ...
        throws ConditionThrowable
      {
        ...
        while (second != NIL)
          {
            Cons temp = new Cons(second.first());
            splice.cdr = temp;
            splice = temp;
            second = second.rest();
          }
        ...
        for (i = 0; i < limit; i++)
          {
            ...
            if (top == NIL)
              continue;
            result = new Cons(top.first());
            splice = result;
            top = top.rest();
            while (top != NIL)
              {
                Cons temp = new Cons(top.first());
                splice.cdr = temp;
                splice = temp;
                top = top.rest();
              }
    ...
    new Primitive(SymbolConstants.MACRO_FUNCTION, "symbol &optional environment")
    {
      ...
      public LispObject execute(LispObject arg) throws ConditionThrowable
      {
        ...
        if (obj instanceof SpecialOperator)
          {
            obj = get(arg, SymbolConstants.MACROEXPAND_MACRO, NIL);
            if (obj instanceof AutoloadMacro)
              {
                ((AutoloadMacro)obj).load();
                obj = get(arg, SymbolConstants.MACROEXPAND_MACRO, NIL);
        ...
        throws ConditionThrowable
      {
        ...
        if (obj instanceof SpecialOperator)
          {
            obj = get(first, SymbolConstants.MACROEXPAND_MACRO, NIL);
            if (obj instanceof AutoloadMacro)
              {
                ((AutoloadMacro)obj).load();
                obj = get(first, SymbolConstants.MACROEXPAND_MACRO, NIL);
  ...
  private static final SpecialOperator DEFMACRO =
    new SpecialOperator(SymbolConstants.DEFMACRO)
    {
        ...
        throws ConditionThrowable
      {
        ...
        if (symbol.getSymbolFunction() instanceof SpecialOperator)
          put(symbol, SymbolConstants.MACROEXPAND_MACRO, macroObject);
        else
  ...
  private static final SpecialOperator COND =
    new SpecialOperator(SymbolConstants.COND, "&rest clauses")
    {
        ...
        throws ConditionThrowable
      {
        ...
        while (args != NIL)
          {
            LispObject clause = args.first();
            result = eval(clause.first(), env, thread);
            thread._values = null;
            if (result != NIL)
              {
                LispObject body = clause.rest();
                while (body != NIL)
                  {
  ...
  private static final SpecialOperator CASE =
    new SpecialOperator(SymbolConstants.CASE, "keyform &body cases")
    {
      ...
      {
        final LispThread thread = LispThread.currentThread();
        LispObject key = eval(args.first(), env, thread);
        args = args.rest();
        while (args != NIL)
          {
            LispObject clause = args.first();
            LispObject keys = clause.first();
            boolean match = false;
            if (keys.isList())
              {
                while (keys != NIL)
                  {
                    LispObject candidate = keys.first();
  ...
  private static final SpecialOperator ECASE =
    new SpecialOperator(SymbolConstants.ECASE, "keyform &body cases")
    {
        ...
        throws ConditionThrowable
      {
        final LispThread thread = LispThread.currentThread();
        LispObject key = eval(args.first(), env, thread);
        LispObject clauses = args.rest();
        while (clauses != NIL)
          {
            LispObject clause = clauses.first();
            LispObject keys = clause.first();
            boolean match = false;
            if (keys.isList())
              {
    ...
    new Primitive(SymbolConstants.APPLY, "function &rest args")
    {
        ...
        throws ConditionThrowable
      {
        ...
        switch (length)
          ...
          case 3:
            return thread.execute(fun, args.first(), args.cadr(),
        ...
        throws ConditionThrowable
      {
        if (third.isList())
          {
            final int numFunArgs = 1 + third.seqLength();
      ...
      public LispObject execute(final LispObject[] args) throws ConditionThrowable
      {
        ...
        if (spread.isList())
          {
            final int numFunArgs = numArgs - 2 + spread.seqLength();
            final LispObject[] funArgs = new LispObject[numFunArgs];
            int j = 0;
    ...
    new Primitive(SymbolConstants.EXPORT, "symbols &optional package")
    {
      ...
      public LispObject execute(LispObject arg) throws ConditionThrowable
      {
        ...
        if (arg instanceof Cons)
          {
            for (LispObject list = arg; list != NIL; list = list.rest())
        ...
        if (first instanceof Symbol)
          {
            ...
            if (sourcePathname != Keyword.TOP_LEVEL)
              put(symbol, SymbolConstants._SOURCE, new Cons(sourcePathname, third));
            else
              put(symbol, SymbolConstants._SOURCE, sourcePathname);
          }
        else if (isValidSetfFunctionName(first))
          {
            ...
            put(symbol, SymbolConstants.SETF_FUNCTION, second);
          }
        else
        ...
        try
          {
            ...
            while (defs != NIL)
              {
                LispObject def = checkList(defs.first());
                Symbol symbol = checkSymbol(def.first());
  ...
  private static final Primitive MAKE_EXPANDER_FOR_MACROLET =
                  ...
                  "definition")
    {
      ...
      public LispObject execute(LispObject definition)
        throws ConditionThrowable
      {
        Symbol symbol = checkSymbol(definition.first());
        ...
        throws ConditionThrowable
      {
        ...
        if (binding == null)
          return error(new ControlError("No tag named " +
                                         args.first().writeToString() +
                                         " is currently visible."));
        throw new Go(args.first());
      }
    };
  ...
  private static final SpecialOperator BLOCK =
    new SpecialOperator(SymbolConstants.BLOCK, "name &rest forms")
    {
        ...
        throws ConditionThrowable
      {
        ...
        LispObject body = ((Cons)args).rest();
        Environment ext = new Environment(env);
    ...
    new SpecialOperator(SymbolConstants.CATCH, "tag &body body")
    {
        ...
        throws ConditionThrowable
      {
        if (args.seqLength() < 1)
          return error(new WrongNumberOfArgumentsException(this));
        final LispThread thread = LispThread.currentThread();
        LispObject tag = eval(args.first(), env, thread);
    ...
    new SpecialOperator(SymbolConstants.THROW, "tag result")
    {
        ...
        throws ConditionThrowable
      {
        if (args.seqLength() != 2)
          return error(new WrongNumberOfArgumentsException(this));
        final LispThread thread = LispThread.currentThread();
        thread.throwToTag(eval(args.first(), env, thread),
  ...
  private static final SpecialOperator EVAL_WHEN =
    new SpecialOperator(SymbolConstants.EVAL_WHEN, "situations &rest forms")
    {
      ...
      public LispObject execute(LispObject args, Environment env)
        throws ConditionThrowable
      {
        LispObject situations = args.first();
        if (situations != NIL)
          {
  ...
  private static final SpecialOperator MULTIPLE_VALUE_BIND =
                        ...
                        "vars value-form &body body")
    {
      ...
      public LispObject execute(LispObject args, Environment env)
        throws ConditionThrowable
      {
        LispObject vars = args.first();
        args = args.rest();
        LispObject valuesForm = args.first();
        LispObject body = args.rest();
        final LispThread thread = LispThread.currentThread();
        ...
        while (specials != NIL)
          {
            Symbol symbol = (Symbol) specials.first();
                        ...
                        "values-form &rest forms")
    {
        ...
        throws ConditionThrowable
      {
        if (args.seqLength() == 0)
          return error(new WrongNumberOfArgumentsException(this));
        final LispThread thread = LispThread.currentThread();
        LispObject result = eval(args.first(), env, thread);
        LispObject[] values = thread._values;
  ...
  private static final SpecialOperator MULTIPLE_VALUE_CALL =
    new SpecialOperator(SymbolConstants.MULTIPLE_VALUE_CALL, "fun &rest args")
    {
        ...
        throws ConditionThrowable
      {
        ...
        final LispThread thread = LispThread.currentThread();
        LispObject function;
        LispObject obj = eval(args.first(), env, thread);
        args = args.rest();
    ...
    new SpecialOperator(SymbolConstants.NTH_VALUE, "n form")
    {
        ...
        throws ConditionThrowable
      {
        if (args.seqLength() != 2)
          return error(new WrongNumberOfArgumentsException(this));
        final LispThread thread = LispThread.currentThread();
        int n = Fixnum.getValue(eval(args.first(), env, thread));
        if (n < 0)
          n = 0;
        ...
        if (first instanceof AbstractVector)
          {
            ...
            return v.subseq(start, v.seqLength());
          }
  ...
  private static final Primitive NRECONC =
    new Primitive(SymbolConstants.NRECONC, "list tail")
    {
        ...
        throws ConditionThrowable
      {
        if (list instanceof Cons)
          {
            LispObject list3 = list.rest();
            if (list3 instanceof Cons)
              {
                if (list3.rest() instanceof Cons)
                  {
                    LispObject list1 = list3;
                    LispObject list2 = NIL;
                    do
                      {
                        LispObject h = list3.rest();
                        list3.setCdr(list2);
                        list2 = list3;
                        list3 = h;
                      } while (list3.rest() instanceof Cons);
                    list.setCdr(list2);
                    list1.setCdr(list3);
                  }
                LispObject h = list.first();
  ...
  private static final Primitive SIMPLE_VECTOR_SEARCH =
    new Primitive("simple-vector-search", PACKAGE_SYS, false)
    {
        ...
        throws ConditionThrowable
      {
        ...
        if (first.seqLength() == 0)
          return Fixnum.ZERO;
        final int patternLength = first.seqLength();
        final int limit = v.seqLength() - patternLength;
        if (first instanceof AbstractVector)
          {
          ...
          {
            // Pattern is a list.
            LispObject element = first.first();
            for (int i = 0; i <= limit; i++)
              {
                if (v.AREF(i).eql(element))
                  {
                    ...
                    // We've already checked the first element.
                    int j = i + 1;
                    for (LispObject rest = first.rest(); rest != NIL; rest = rest.rest())
                      {
