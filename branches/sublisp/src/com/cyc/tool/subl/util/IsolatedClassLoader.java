...
public class IsolatedClassLoader extends URLClassLoader {
    ...
    for (int i = 0; i < innersMaybe.length; i++) {
      ...
      if (named.startsWith(className)) {
        try {
          ...
          Errors.error("Error while loading inner classes for: " + className, new SubLException(t));
        }
      }
    }
  }
  ...
  public void addJarClass(final File jfile, String className) throws IOException {
    ...
    addJar(jfile);
    JarURLConnection jarConnection = (JarURLConnection) jfile.toURL().openConnection();
