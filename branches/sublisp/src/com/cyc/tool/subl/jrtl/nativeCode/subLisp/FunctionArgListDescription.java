...
public final class FunctionArgListDescription implements CommonSymbols {
  ...
  public static class OptionalArgDescription {
    ...
    public OptionalArgDescription(SubLObject desc) {
      ...
      if (desc.isCons()) {
        ...
        if (advancedDesc.size() > 3) {
          throw new SubLException("Too many arguments for optional argument: " + advancedDesc);
        }
        init(name, defaultValue, wasSetArgName);
        return;
      }
      throw new SubLException("Got invalid type for optional argument: "
          + desc.toTypeName() + "\nValue: " + desc);
    }
    
    public void init(SubLObject argName, SubLObject defaultValue, SubLObject wasSetArgName) {
      if (!isAllowableArgSymbol(argName)) {
        throw new SubLException("Formal optional argument name expected type SYMBOL, got: " + argName.toTypeName());
      }
  ...
  private void parseFormalArgs(SubLObject args) {
    if (!args.isList()) {
      throw new SubLException("Invalid formal argument list type.\n"
    ...
    try {
      ...
      if ((i < size) && isOptionalSymbol(argArray[i])) {
        ...
        if ((i < size) && isOptionalDeclaration(argArray[i])) {
          ...
          throw new SubLException("&OPTIONAL present but no optional arguments given.\nValue: " + args);
        }
      ...
      if ((i < size) && isRestSymbol(argArray[i])) {
        ...
        if ((i < size) && isAllowableArgSymbol(argArray[i])) {
          ...
          throw new SubLException("&REST present but no variable given.\nValue: " + args);
        }
      }
      if (curArg == (size - 1)) { return; }
      throw new SubLException("Invalid formal argument list: " + args);
    } finally {
  ...
  private void addRequiredArg(SubLObject symbol) {
    if (!isAllowableArgSymbol(symbol)) {
      throw new SubLException("Formal required argument name expected type SYMBOL, got: " + symbol.getClass());
    }
    args.add(symbol);
  }
  ...
  private void setRestSymbol(SubLObject symbol) {
    if (!isAllowableArgSymbol(symbol)) {
      throw new SubLException("Formal rest argument name expected type SYMBOL, got: " + symbol.getClass());
    }
  ...
  private List optionals = new ArrayList();
