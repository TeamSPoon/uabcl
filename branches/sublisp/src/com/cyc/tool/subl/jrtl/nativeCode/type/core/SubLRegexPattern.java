...
public final class SubLRegexPattern extends AbstractSubLObject implements SubLObject, CommonSymbols {
  ...
  public SubLRegexPattern(String patternStr, int options) {
    ...
    if ((options & REGEX_COMPOPT_ANCHORED) != 0) {
      throw new SubLException("*regex-comopt-anchored* is not supported.");
    }
    ...
    if ((options & REGEX_COMPOPT_DOLLAR_END_ONLY) != 0) {
      throw new SubLException("*regex-compopt-dollar-endonly* is not supported.");
    }
    ...
    if ((options & REGEX_COMPOPT_EXTRA) != 0) {
      throw new SubLException("*regex-compopt-extra* is not supported.");
    }
    if ((options & REGEX_COMPOPT_UNGREEDY) != 0) {
      throw new SubLException("*regex-compopt-ungreedy* is not supported.");
    }
    if ((options & REGEX_COMPOPT_UTF8) != 0) {
      ; //ignore
    }
    if ((options & REGEX_COMPOPT_NO_AUTO_CAPTURE) != 0) {
      throw new SubLException("*regex-compopt-no-auto-capture* is not supported.");
    }
    if ((options & REGEX_COMPOPT_NO_UTF8_CHECK) != 0) {
      throw new SubLException("*regex-compopt-no-utf8-check* is not supported.");
    }
  ...
  public ArrayList getAllMatches(CharSequence str, int options, int maxNumMatches) {
    if (options != 0) {
      throw new SubLException("No options supported on regex match under this implementation.");
    }
    matcher.reset(str);
  ...
  public ArrayList getAllMatchesOffsets(CharSequence str, int options, int maxNumMatches) {
    if (options != 0) {
      throw new SubLException("No options supported on regex match under this implementation.");
    }
    matcher.reset(str);
