...
public abstract class AbstractSubLStream extends AbstractSubLObject implements SubLStream {
  ...
  public final String getFileDesignator() {
    if (!isRandomAccess()) {
      throw new SubLException("Invalid pathname designator: " + this + ".");
    }
    File file = getFile();
    if (file == null) {
      throw new SubLException("Invalid pathname designator: " + this + ".");
  ...
  private void init(SubLSymbol elementType, SubLSymbol direction,
    ...
    if ((elementType != TEXT_KEYWORD) && (elementType != BINARY_KEYWORD)) {
      throw new SubLException("Invalid stream element type: " + elementType);
    }
    this.direction = direction;
    if ((direction != INPUT_KEYWORD) && (direction != OUTPUT_KEYWORD)
        && (direction != IO_KEYWORD) && (direction != PROBE_KEYWORD)) {
      throw new SubLException("Invalid stream direction: " + direction);
    }
    ...
    if ((ifExists != ERROR_KEYWORD) && (ifExists != NEW_VERSION_KEYWORD)
      ...
      throw new SubLException("Invalid stream if-exists parameter: " + ifExists);
    }
    this.ifNotExists = ifNotExists;
    if (((ifNotExists != ERROR_KEYWORD) && (ifNotExists != CREATE_KEYWORD)) && (ifNotExists != NIL)) {
      throw new SubLException("Invalid stream if-not-exists parameter: " + ifNotExists);
    }
    if (ifExists == RENAME_AND_DELETE_KEYWORD) {
      throw new SubLException("Streams implementation does not currently support :rename-and-delete.");
    }
    if (direction == PROBE_KEYWORD) {
      throw new SubLException("Streams implementation does not currently support :probe.");
    }
  }
