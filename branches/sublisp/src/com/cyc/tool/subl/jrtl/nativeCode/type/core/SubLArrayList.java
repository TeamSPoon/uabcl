...
public final class SubLArrayList extends AbstractSubLList implements SubLCons, SubLList {
  ...
  SubLArrayList(int capacity) {
    this.theList = new ArrayList(capacity);
  }
  ...
  SubLArrayList(ArrayList theList, boolean reuseArray) {
    if (reuseArray) {
      this.theList = theList;
    } else {
      this.theList = (ArrayList)theList.clone();
    }
  }
  ...
  public final SubLObject makeCopy() {
    ...
    ArrayList newArrayList = new ArrayList(Arrays.asList(newItems));
  ...
  public final SubLObject last(int n) {
    if (n < 0) { 
      throw new SubLException("Got bad value to function 'last': " + n);
    }
    if (n == 0) {
  ...
  public Object[] toArray() {
    if (getDottedElement() != SubLNil.NIL) {
      throw new SubLException("Unable to convert inproper lists to arrays: " + this);
    }
  ...
  public SubLObject[] toSubLObjectArray() {
    if (!isProper()) {
      throw new SubLException("Can't convert an inproper list to array.");
    }
  ...
  public SubLObject nth(int index) {
    if (index < 0 ) { throw new SubLException("Got invalid index to nth: " + index); }
    int size = size();
    SubLObject dottedElement = getDottedElement();
    if (index == size) {
      if (!dottedElement.isList()) {
        throw new SubLException("Unable to find element " + index + " in list: " + this);
      }
  ...
  public SubLObject nthCdr(int index) {
    throw new SubLException("This shouldn't be possible.");
  ...
  public int listLength() {
    throw new SubLException("Implement me!");
  }
  ...
  public SubLCons setRest(SubLObject rest) {
    try {
      ...
      throw new SubLException("Error making cons.", e);
    }
  }
  ...
  public SubLSequence sort(boolean isDestructive, final BinaryFunction pred,
    ...
    if (!isProper()) {
      throw new SubLException("Can't sort improper list.");
    }
    if (this.size() == 1) {
      if (isDestructive) {
        return this;
    ...
    Object[] seq = toArray();
    if (key == IDENTITY_UNARY_FUNC) {
      Arrays.sort(seq, new ComparatorIdentityKey(pred));  //@todo get rid of new
    } else {
      Arrays.sort(seq, new ComparatorGenericKey(pred, key));  //@todo get rid of new
    }
    if (isDestructive) {
  ...
  public final SubLSequence makeSequenceFromJavaList(List newBuf) {
    ...
    ArrayList theBuf = new ArrayList(size);
