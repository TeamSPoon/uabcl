...
public final class SubLConsPair extends AbstractSubLList implements SubLCons, SubLList, CommonSymbols{
  ...
  public SubLSequence sort(boolean isDestructive, final BinaryFunction pred, final UnaryFunction key) {
    ...
    try {
      ...
      if (key == IDENTITY_UNARY_FUNC) {
        Arrays.sort(seq, new ComparatorIdentityKey(pred)); // @todo resource this
      } else {
        Arrays.sort(seq, new ComparatorGenericKey(pred, key)); // @todo resource this
      }
      if (isDestructive) {
        int i = 0;
        for (SubLObject cur = this; cur != SubLNil.NIL; cur = cur.rest()) {
  ...
  public final SubLSequence merge(SubLSequence otherSeq, BinaryFunction test, UnaryFunction key) {
    ...
    SubLCons result = SubLObjectFactory.makeCons(SubLNil.NIL, SubLNil.NIL);
    SubLObject next = result;
    SubLObject seq1 = this;
    SubLObject seq2 = otherSeq;
    while ((seq1 != SubLNil.NIL) && (seq2 != SubLNil.NIL)) {
      SubLObject key1 = key.processItem(seq1.first());
      SubLObject key2 = key.processItem(seq2.first());
      if (test.processItem(key1, key2) != SubLNil.NIL) {
        next.setFirst(seq1.first());
        seq1 = seq1.rest();
      } if (test.processItem(key2, key1) != SubLNil.NIL) {
        next.setFirst(seq2.first());
        seq2 = seq2.rest();
      } else {
        next.setFirst(seq1.first());
        seq1 = seq1.rest();
      }
      if ((seq1 != SubLNil.NIL) && (seq2 != SubLNil.NIL)) {
        next.setRest(SubLObjectFactory.makeCons(SubLNil.NIL, SubLNil.NIL));
        next = next.rest();
      }
    }
    if (seq1 == SubLNil.NIL) {
