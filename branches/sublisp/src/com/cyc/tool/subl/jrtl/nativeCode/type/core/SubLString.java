...
public final class SubLString extends AbstractSubLArraySequence implements SubLObject, SubLSequence, Cloneable, CharSequence  {
  ...
  SubLString(int length, char defaultChar) {
    if (size < 0) {
      throw new SubLException("Attempt to create string with negative size: " + size);
    }
    buf = new char[length];
    size = length;
  ...
  public SubLSequence sort(boolean isDestructive, final BinaryFunction pred,
    ...
    if (key == IDENTITY_UNARY_FUNC) {
      Arrays.sort(chars, new ComparatorIdentityKey(pred));  //@todo get rid of new
    } else {
      Arrays.sort(chars, new ComparatorGenericKey(pred, key));  //@todo get rid of new
    }
